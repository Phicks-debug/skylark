// GemmaCLI - C++ CLI for running Gemma models with LiteRT-LM
...
static void print_help(const char* prog) {
    std::cout << R"(GemmaCLI - Interactive CLI for Gemma models via LiteRT-LM
...
Model Options:
  ...
  --cache-dir DIR        Cache directory for downloaded models
                         (default: ~/.cache/gemma-cli/models)
...
static void chat_loop(const Config& cfg) {
    ...
    cprintln("GemmaCLI - Type your message (Ctrl+C to cancel, Ctrl+D to exit)",
            Color::BrightWhite);
    ...
    // ---- Chat loop ----
    std::string line;
    std::string audio_file = "/tmp/gemma_cli_recording.wav";
